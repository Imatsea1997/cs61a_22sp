...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        "*** YOUR CODE HERE ***"
        """" 算出表达式的值 存入frame里 """
        value = scheme_eval(expressions.rest.first, env)        # expr只有一个rest，rest.first才是表达式
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        """" 把构造好的lambdaProcedure 存入 f里"""
        formals = signature.rest
        body = expressions.rest
        procedure = do_lambda_form(Pair(formals, body), env)    # 这句话很强。。数据结构的理解
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    #print('DEBUG:define lambda, expr is: ', expressions)
    body = expressions.rest
...
def do_and_form(expressions, env):
    ...
    while operands != nil:
        last_value = scheme_eval(operands.first, env)
...
def do_cond_form(expressions, env):
    ...
    return None     # no predicts & no else
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings != nil:
        temp_binding = bindings.first
        validate_form(temp_binding, 2, 2)   # 结构是不是(a 2) each binding
        temp_name = temp_binding.first
        temp_val = scheme_eval(temp_binding.rest.first, env)

        names = Pair(temp_name, names)
        vals = Pair(temp_val, vals)
        
        bindings = bindings.rest
    validate_formals(names) # 所有let的binding 是不是没有重复赋值的情况
    # END PROBLEM 14
