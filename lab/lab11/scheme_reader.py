...
def scheme_read(src):
    ...
    if val == 'nil':                     # 读完了
        return nil
    elif val not in DELIMITERS:          # numbers booleans symbols
        return val
    elif val == "'":                     # 特殊符号'的处理
        return Pair('quote', Pair(scheme_read(src), nil))
    elif val == '(':                     # recursive case：sub expression
...
def read_tail(src):
    ...
    try:
        while src.end_of_line():
            src.pop_first()
        if src.current is None:                     # 异常情况：sub expression不应该不存在
            raise SyntaxError('unexpected end of file')
        elif src.current == ')':                    # 读完了sub expression
            "*** YOUR CODE HERE ***"
            src.pop_first()
            return nil
        else:                                      # recursive case：分解问题：first + rest来存储
            "*** YOUR CODE HERE ***"
