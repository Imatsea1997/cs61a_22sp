...
def pingpong(n):
    ...
    True
    """
    def pingpong_helper(index, increase_step):
        # base case
        if index > n:
            return 0
        # recursive case
            ...
            if index % 8 != 0 and num_eights(index) == 0:
                return increase_step + pingpong_helper(index + 1, increase_step)
            # change增长方式
            else:
                return increase_step + pingpong_helper(index + 1, -1 * increase_step)
